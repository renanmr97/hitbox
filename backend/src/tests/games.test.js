...
// Limpa os jogos de teste após todos os testes rodarem
afterAll(async () => {
...
describe("GET /games/:id", () => {
  ...
  beforeAll(async () => {
    const res = await request(app).post("/games").send({
      title: "[TESTE] Jogo para busca por ID",
    });
    gameId = res.body.id;
  });
...
describe("PUT /games/:id", () => {
  ...
  beforeAll(async () => {
    const res = await request(app).post("/games").send({
      title: "[TESTE] Jogo para atualizar",
    });
    gameId = res.body.id;
  });

  it("deve atualizar o jogo e retornar status 200", async () => {
    const res = await request(app).put(`/games/${gameId}`).send({
      title: "[TESTE] Jogo atualizado",
    });
    expect(res.statusCode).toBe(200);
